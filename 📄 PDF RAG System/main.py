"""
PDF RAG Assistant (RAG System)
==============================

Main pipeline for RAG-based question answering using PDF documents:
- PDF document ingestion with OCR fallback
- Vector database creation
- Interactive Q&A with Groq AI

This is the main entry point for the PDF RAG assistant.

Usage:
    python main.py

The system will:
1. Process PDF documents from the data/ folder
2. Extract text with OCR fallback for image-based PDFs
...
# Import modules
from ingestion.ingest_pdfs import get_all_chunks, process_pdf_documents
...
from utils.helpers import (
    log_step, format_time_elapsed, ensure_directory_exists,
...
{'=' * 70}
🤖 PDF RAG ASSISTANT
   PDF Processing with Groq AI
   Production Ready + Free Tier
{'=' * 70}

This system will:
📥 1. Process PDF documents with OCR fallback
...
🔍 Grounded answers only - if information isn't in the docs,
   the system will say: 'The knowledge base does not contain this information.'
    ...
    def get_pdf_files(self) -> List[str]:
        """
        Gets PDF files from the data directory.
        
        Returns:
            List[str]: List of PDF file paths
        """
        print("📋 STEP 1: PDF Document Discovery")
        print("-" * 40)
        
        # Ensure data directory exists
        ensure_directory_exists(DATA_DIR)
        
        # Search for PDF files in data directory
        pdf_files = []
        ...
        if not pdf_files:
            ...
            print("\nExample PDF files you can add:")
            print("  - data/research_paper.pdf")
            print("  - data/manual.pdf")
            print("  - data/documentation.pdf")
            return []
        
        print(f"\n✅ Found {len(pdf_files)} PDF file(s)")
        return pdf_files
    
    def run_ingestion(self, pdf_files: List[str]) -> bool:
        """
        Runs the PDF document ingestion process.
        
        Args:
            pdf_files (List[str]): PDF file paths to process
            
        Returns:
            bool: True if successful
        """
        log_step("Starting PDF document ingestion", 1)
        start = time.time()
        ...
        if results:
            self.processed_files = list(results.keys())
            ...
            print("❌ Ingestion failed - no PDF documents were processed successfully")
            return False
    ...
    def show_final_summary(self):
        ...
        print(f"📄 PDFs processed: {len(self.processed_files)}")
    ...
    def run(self):
        ...
        try:
            ...
            if not pdf_files:
                print("❌ Cannot continue without PDF files")
                return
            
            # Step 2: Ingest PDF documents
            if not self.run_ingestion(pdf_files):
